    ...
    while True:
        try:
            ...
            if "BANNED" in str(req_game.text) or req_game.status_code == 403:
                ...
                print(f"{white}[{green}{logTime()}{white}] {yan}Nickname: {green}{req_game.json()['User']['Username']} {white}| Country: {green}{req_game.json()['User']['Country']} {white}| Trophy: {green}{req_game.json()['User']['SkillRating']} {white}| Crown: {green}{req_game.json()['User']['Crowns']}")
        except:
            continue
